...
//     "https://demo-asto-poject-vrxc.vercel.app",
//     "https://demo-asto-poject.vercel.app",
//     "https://demo-asto-poject-lnet.vercel.app",
//     "https://astrologer-backendcoll-chaat.onrender.com"
// ];
...
//     // ✅ 6. ICE Candidate forwarding for WebRTC (ADD THIS)
...
io.on('connection', (socket) => {
    ...
    // ✅ 5. Video Call Signaling - FIXED
    socket.on('call-user', (data) => {
        console.log(`📞 [DEBUG] Call Signal from ${data.from} to ${data.to}`);
        console.log(`📞 [DEBUG] Signal data:`, data.signal);
        
        const targetSocket = users[String(data.to)];
        if (targetSocket) {
            // ✅ CRITICAL FIX: Only send incoming-call with the actual signal
            io.to(targetSocket).emit('incoming-call', { 
                from: data.from, 
                signal: data.signal  // Keep the original WebRTC offer signal
            });
            console.log(`✅ Incoming-call sent to ${data.to} with signal`);
        } else {
            console.log(`❌ User ${data.to} not online`);
        }
    });
    ...
    // ✅ 6. Answer Call - FIXED
    socket.on('answer-call', (data) => {
        console.log(`📞 [DEBUG] Answer Call from ${socket.id} to ${data.to}`);
        console.log(`📞 [DEBUG] Answer signal:`, data.signal);
    ...
    // 7. End Call
    socket.on('end-call', (data) => {
        console.log(`🔴 [DEBUG] End Call to ${data.to}`);
        const targetSocket = users[String(data.to)];
        if (targetSocket) {
            io.to(targetSocket).emit('call-ended');
            console.log(`🔴 Call ended with ${data.to}`);
    ...
    // 8. ICE Candidate forwarding for WebRTC
    socket.on('ice-candidate', (data) => {
        const targetSocket = users[String(data.to)];
        if (targetSocket) {
            io.to(targetSocket).emit('ice-candidate', {
